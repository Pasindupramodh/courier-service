...
$managerId = addsLashes($_POST["managerId"]);
...
$db = new Database();
$arr = array();
$arr['emp_id'] = $managerId;
$query = "select * from employee where emp_id = :emp_id";
$check = $db->db_check($query, $arr);
if ($check) {
    echo "manager ID Already Exists";
} else {
    $arr = array();
    $arr['nic'] = $managerId;
    ...
    $check = $db->db_check($query, $arr);
    if ($check) {
        echo "manager NIC Already Exists";
    } else {
        $arr = array();
        $arr['email'] = $managerId;
...
// Database::iud("INSERT INTO `student`(`brand_id`,`model_id`) VALUES ('".$brand."','".$model."')");

// $model_st = Database::search("SELECT count(manager_id) as count FROM `manager`");

// $model_data = $model_st->fetch_assoc();
// $studentId = "A0" . ((int)$model_data["count"] + 1);
// $enc_password = password_hash($password, PASSWORD_DEFAULT);

// $model_st = Database::search("SELECT count(user_id) as count FROM `login`");
// $model_data = $model_st->fetch_assoc();
// $userId = "U0" . ((int)$model_data["count"] + 1);

// $model_st = Database::search("SELECT count(verifi_id) as count FROM `verificcation_code`");
// $model_data = $model_st->fetch_assoc();
// $verify_id = "V0" . ((int)$model_data["count"] + 1);

// $mhb_rs = Database::search("SELECT * FROM `manager` WHERE email = '" . $email . "'");

// $mhb_rs = Database::search("SELECT * FROM `manager` WHERE email = '" . $email . "'");
// if ($mhb_rs->num_rows > 0) {
//     echo "manager Email Already Exists";
// } else {
//     $mhb_rs = Database::search("SELECT * FROM `manager` WHERE nic = '" . $nic . "'");
//     if ($mhb_rs->num_rows > 0) {

//         echo "manager NIC Already Exists";
//     } else {

//         $mhb_rs = Database::search("SELECT * FROM `login` WHERE user_name = '" . $uname . "'");
//         if ($mhb_rs->num_rows > 0) {

//             echo "Username Already Exists";
//         }else{
//             $sql = "INSERT INTO `verificcation_code` (`verifi_id`,`verificcation_code`,`is_used`) 
//             VALUES ('" . $verify_id . "','" . $unique_code . "','0');";
//         $sql .= "INSERT INTO `login` (`user_id`,`user_name`,`password`,`user_type`,`is_verified`,`active_status`,`verificcation_code_verifi_id`)
//             VALUES ('" . $userId . "','" . $uname . "','" . $enc_password . "','manager','0','1','" . $verify_id . "');";

//         $sql .= "INSERT INTO `manager`(`manager_id`,`fname`,`lname`,`nic`,`mobile`,`email`
//                 ,`gender`,`dob`,`address_line_1`,`address_line_2`,`address_line_3`,
//                 `active_status`,`login_user_id`) 
//                 VALUES ('" . $studentId . "','" . $fname . "','" . $lname . "','" . $nic . "','" . $mobile . "','"
//             . $email . "','" . $gender . "','" . $bday . "','" . $address1 . "','" . $address2
//             . "','" . $address3 . "','1','" . $userId . "')";
        ...
        // $mail->SMTPAuth = true;
        // $mail->Username = 'pasindupramodh6@gmail.com';
        // $mail->Password = 'thrycttbpqmdzwhu';
        // $mail->SMTPSecure = 'ssl';
        // $mail->Port = 465;
        // $mail->setFrom('pasindupramodh6@gmail.com', 'School');
        // $mail->addReplyTo('pasindupramodh6@gmail.com', 'School');
        // $mail->addAddress($email);
        // $mail->isHTML(true);
        // $mail->Subject = 'Login Details';
        // $bodyContent = '<h3>Dear ' . $fname . ' ' . $lname . ',</h3>

        // <p>Thank you for enrolling as a manager of the School - Student Online Learning Advancement Portal. Now you have completed your manager enrolment and your logon account for the Student Network will be activated within one hour.</p>
