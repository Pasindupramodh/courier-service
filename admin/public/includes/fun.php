...
class Employee extends Database
{
	...
	public function getReturnedCouriers(){
        ...
        tracking on courier.courier_id = tracking.courier_id inner join 
        courier_status on courier_status.courier_status_id = tracking.courier_status_id
		where courier_status.status ='Returned';";
		return $this->db_read($query);
	}
	public function getBarcode(){
		$query = "SELECT * FROM customer inner join barcode on customer.barcode_barcode_id = barcode.barcode_id;";
		return $this->db_read($query);
	}
	public function barcodeMax(){
		$query = "SELECT  max(barcode_end) as max FROM barcode;";
	...
	public function getPendingCustomers(){
		$query ="SELECT * FROM customer inner join user on user.user_id = customer.user_id where status_id=2 and is_verify = 0;";
		return $this->db_read($query);
	}
	public function getBlockedCustomers(){
		$query ="SELECT * FROM customer inner join user on user.user_id = customer.user_id where status_id=2 and is_verify = 1;";
		return $this->db_read($query);
	}
	public function getCustomers(){
		$query ="SELECT * FROM customer inner join user on user.user_id = customer.user_id where status_id=1 and is_verify = 0;";
		return $this->db_read($query);
	}
	public function getPendingCustomersCount(){
		$query = "SELECT count(user_id) as count FROM user where status_id=2 and is_verify = 0 and user_type_id = 4";
		$result = $this->db_read($query)[0];
		$count = $result['count'];
		return $count;
	}
	public function getBank($branch){
		$query ="SELECT * FROM bank_branch inner join bank on bank_branch.bank_id = bank.bank_id where branch_id = $branch;";
		return $this->db_read($query);
	}
}
