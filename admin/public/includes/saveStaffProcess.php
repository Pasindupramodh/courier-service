...
if ($check) {
    ...
    if ($check) {
        ...
        $check = $db->db_check($query, $arr);
        if ($check) {
            echo "Staff Email Already Exists";
        } else {
            $arr = array();
            $arr['user_name'] = $staffId;
...
// $model_st = Database::search("SELECT count(staff_id) as count FROM `staff`");
...
// $mhb_rs = Database::search("SELECT * FROM `staff` WHERE email = '" . $email . "'");

// $mhb_rs = Database::search("SELECT * FROM `staff` WHERE email = '" . $email . "'");
// if ($mhb_rs->num_rows > 0) {
//     echo "staff Email Already Exists";
// } else {
//     $mhb_rs = Database::search("SELECT * FROM `staff` WHERE nic = '" . $nic . "'");
//     if ($mhb_rs->num_rows > 0) {
...
//             VALUES ('" . $userId . "','" . $uname . "','" . $enc_password . "','staff','0','1','" . $verify_id . "');";

//         $sql .= "INSERT INTO `staff`(`staff_id`,`fname`,`lname`,`nic`,`mobile`,`email`
...
//             . "','" . $address3 . "','1','" . $userId . "')";
        ...
        // <p>Thank you for enrolling as a staff of the School - Student Online Learning Advancement Portal. Now you have completed your staff enrolment and your logon account for the Student Network will be activated within one hour.</p>
