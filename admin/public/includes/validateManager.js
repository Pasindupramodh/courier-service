const schema = joi.object({
    manager_id: joi.string().min(3).max(30).label("Manager ID").required(),
...
$(document).ready(function () {
    $(".formManager").on("submit", function (e) {
        e.preventDefault();
        const form = this;
        const managerIdField = $(form).find("#manager_id");
        ...
        if (isValid) {
            ...
            var e = document.getElementById("manager_type");
            ...
            var f = new FormData();
            f.append("managerId", manager_id.value);
            ...
            r.open("POST", "saveManagerProcess.php", true);
            r.send(f);
